...
interface IntroSurahProps {
    introSurah: string | null;
}

const IntroSurahDialog = ({ introSurah }: IntroSurahProps) => {
    const [isOpen, setIsOpen] = useState(false);

    if (!introSurah) return null;
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="max-w-md lg:max-w-2xl">
                <DialogHeader>
                    <DialogTitle>Pengantar Surah</DialogTitle>
                </DialogHeader>

                <div className="max-h-[70vh] overflow-y-auto [mask-image:linear-gradient(to_bottom,black_80%,transparent_100%)] pb-16">
                    <p className="text-sm leading-relaxed whitespace-pre-line text-gray-300" dangerouslySetInnerHTML={{ __html: introSurah }} />
                </div>
...
export default IntroSurahDialog;
