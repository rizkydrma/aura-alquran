...
const DzikirPage: React.FC = ({}) => {
    const { data, isLoading, isError, error, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteDzikirs({ limit: 10 });
            ...
            {data?.pages.map((page, i) => (
                <div key={i} className="grid grid-cols-1 gap-3 space-y-3">
                    {page.data.map((dzikir) => (
                        ...
                        >
                            <div>
                                <h3 className="font-semibold text-gray-800 dark:text-gray-100">{dzikir?.arab}</h3>
                                <h3 className="font-semibold text-gray-800 dark:text-gray-100">{dzikir?.indo}</h3>
                                <h3 className="font-semibold text-gray-800 dark:text-gray-100">{dzikir?.type}</h3>
                                <h3 className="font-semibold text-gray-800 dark:text-gray-100">{dzikir?.repeat}X</h3>
                            </div>
                        </div>
                    ))}
                </div>
