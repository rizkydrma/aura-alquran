...
interface SidebarDoaProps {
    source: string;
}

const SidebarDoa = ({ source }: SidebarDoaProps) => {
    const { data, isLoading, isError, error } = useDoaGroupedBySource({});
            ...
            <div className="pb-14] h-[80dvh] space-y-4 overflow-y-auto [mask-image:linear-gradient(to_bottom,black_80%,transparent_100%)]">
                {isLoading ? (
                    ...
                    <div className="flex flex-col gap-3">
                        {data?.data?.map((doa) => {
                            const isActive = source == doa?.source;

                            return (
                                <Link key={doa?.source} prefetch href={`/products/doa/${doa?.source}`}>
                                    <SpotlightCard
                                        className={cn(
                                    ...
                                    >
                                        <div className="flex flex-col gap-1">
                                            <h3 className="text-sm font-semibold text-gray-800 capitalize dark:text-gray-100">{doa?.source}</h3>
                                            <p className="text-xs text-gray-500 dark:text-gray-400">{doa?.total_doa} Doa</p>
                                        </div>
